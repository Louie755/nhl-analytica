...
def fetch_nhl_safe(url, season, game_type, sort_prop):
    ...
    start, limit = 0, 100
    while True:
        # gameTypeId=2(정규), 3(플옵)을 동적으로 처리
...
def get_nhl_data():
    now = datetime.now()
    # 4월 현재 데이터가 가장 확실한 20242025를 기본값으로 설정
    ...
    <body>
        ...
        <script>
            ...
            function render() {
                ...
                // [유지] Louie님의 렌더링 방식 (100명까지 즉시 노출)
